...
class Snake():
  ...
  def __init__(self, win_size, wall_size):
    """ Constructor """
  ...
  def place_wall(self):
    ...
    self._wall.append(self._fruit) # Force generation
    while True in [item == self._fruit or item in self._body for item in self._wall]:
      ...
      # Generate wall
      self._wall = []
      for i in range(0, self._wall_size):
        self._wall.append(((x + i * dx)%self._board, (y + i * dy)%self._board))
  ...
  def move(self, direction):
    ...
    self._dead = head in self._body or head in self._wall
  ...
  def get_wall(self):
    """ Method returning the wall """
    return self._wall

def start_game(board_size=50, direction='UP', wall_size=0):
  ...
  # Wall piece - image 10x10 pixels black
  wall_piece = pygame.Surface((10, 10))
  wall_piece.fill(COLOR_BOARD)
  for i in range(0, 5): wall_piece.fill(COLOR_WALL, (2 * i, 0, 1, 10))

  pygame.time.Clock()
  pygame.time.set_timer(1, 100)

  snake = Snake(board_size, wall_size)
  lost  = False
  pause = False
  ...
  while not lost:
    ...
    # Repaint wall
    for bit in snake.get_wall():
      board.blit(wall_piece, (bit[0] * 10, (board_size - bit[1] - 1) * 10))
